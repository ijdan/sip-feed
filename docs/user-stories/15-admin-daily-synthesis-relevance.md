...
- **Synthèses affichées** : la page Synthèse liste les **N dernières synthèses existantes** (tri par ID de document — date ISO — décroissant), et non plus les 3 derniers jours calendaires : une synthèse générée a posteriori pour une date ancienne apparaît donc dans la liste. N est réglable dans l'encart (`settings/global.synthesis_display_count`, défaut 3, borné 1-30 côté backend). Remplace la règle « 3 derniers jours » de l'US-SYN-003 (cf. `09-admin-synthesis.md`).
